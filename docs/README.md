# paragu-ai-platform docs

| File | Purpose |
|---|---|
| `ARCHIVED_REPOS.md` | History of repos that have been archived or merged |
| `BRANCH_PROTECTION.md` | GitHub branch protection rules |
| `CLEANUP_SUMMARY.md` | Summary of past cleanup work |
| `CLEANUP_VERIFICATION.md` | Verification of cleanup completion |
| `DEPLOY_STATUS.md` | Current deployment status of all apps |
| `PHASE_6_MIGRATION_PLAN.md` | Phase 6 migration plan |
| `SITES_VS_MONOREPO_AUDIT.md` | Per-site vs monorepo architecture audit |
| `NEXA_AUDIT_CORRECTION.md` | Correction to the 2026-06-15 audit (5 apps thought broken have content elsewhere) |
| `PRE_EXISTING_BUILD_FAILURES.md` | Pre-existing build failures in platform apps (discovered 2026-06-15 baseline check) |
