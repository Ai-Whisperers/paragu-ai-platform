...
export default async function Servicios({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
      <h1 className="text-4xl sm:text-6xl font-bold mb-3 text-center">{c.title || "Servicios"}</h1>
      ...
      {c.tabs && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {c.tabs.map((t: any) => (
            <Link key={t.id} href={`${base}/servicios#${t.id}`} className="p-4 bg-gray-50 border border-gray-200 rounded-lg hover:bg-white transition-colors flex items-center justify-between">
