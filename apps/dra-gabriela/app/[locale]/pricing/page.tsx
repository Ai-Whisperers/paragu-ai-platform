...
const CATEGORY_TITLES_EN: Record<string, string> = {
  ...
  operatoria: "Operative (Fillings)",
  endodoncia: "Endodontics",
  exodoncia: "Extractions",
  protesis: "Prosthesis",
  implantes: "Implants",
  estetica: "Aesthetic",
  odontopediatria: "Pediatric",
  corporate_note: "Corporate",
...
export default async function Pricing({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
      <h1 className="text-4xl sm:text-6xl font-bold mb-3">{c.title || "Pricing"}</h1>
      {c.metaDescription && <p className="text-gray-600 mb-4">{c.metaDescription}</p>}
      {p.usd_approx && <p className="text-sm text-gray-500 mb-2">Reference: {p.usd_approx}</p>}
      ...
      <div className="space-y-8">
        {categoryOrder.map((key) => {
          ...
          return (
            <section key={key}>
              <h2 className="text-2xl font-bold mb-4 pb-2 border-b border-gray-200">{titles[key] || key}</h2>
        ...
        {p.corporate_note && (
          <section className="p-6 bg-gray-50 rounded-xl">
            <h2 className="text-xl font-bold mb-2">{locale === "es" ? "Corporativo" : "Corporate"}</h2>
        ...
        {p.payment && Array.isArray(p.payment) && (
          <section>
            <h2 className="text-2xl font-bold mb-4 pb-2 border-b border-gray-200">{locale === "es" ? "Opciones de Pago" : "Payment Options"}</h2>
      ...
      {p.cta_whatsapp && (
        <div className="mt-12 p-8 bg-gradient-to-br from-[var(--accent)] to-[var(--accent-2)] text-white rounded-2xl text-center">
          <h2 className="text-2xl font-bold mb-3">{locale === "es" ? "¿Listo para reservar?" : "Ready to book?"}</h2>
