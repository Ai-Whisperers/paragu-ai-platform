      ...
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          ...
          <div className="relative">
            ...
            <div className="absolute -top-4 -right-4 w-24 h-24 rounded-full border-2 border-[var(--gold)] opacity-30 -z-10" />
            <div className="absolute -bottom-4 -left-4 w-16 h-16 rounded-full border-2 border-[var(--accent)] opacity-20 -z-10" />
            {/* Floating badge */}
            <div className="absolute -bottom-6 -right-6 md:right-6 card p-3 hidden md:flex items-center gap-2.5 max-w-[180px]">
              <div className="w-10 h-10 rounded-full bg-[var(--gold-soft)] flex items-center justify-center flex-shrink-0">
                <Award className="w-5 h-5 text-[var(--gold-2)]" />
              </div>
              <div>
                <div className="text-[10px] uppercase tracking-wider text-[var(--fg-subtle)] font-semibold">{isEs ? "Inscripta" : "Registered"}</div>
                <div className="text-xs font-medium text-[var(--fg)]">MSPBS · {isEs ? "Paraguay" : "Paraguay"}</div>
              </div>
            </div>
          </div>
          ...
          <div className="text-left">
              ...
              {BADGES.map((b, i) => {
                ...
                return (
                  <div key={i} className="card p-3 flex flex-col items-center text-center">
                    <Icon className="w-4 h-4 text-[var(--gold)] mb-1.5" />
            ...
            {/* Quote highlight */}
            <blockquote className="border-l-4 border-[var(--gold)] pl-5 py-2 mb-8">
              <p className="text-base italic text-[var(--fg)] leading-relaxed">
                {isEs
              ...
              <footer className="text-xs text-[var(--fg-muted)] mt-2">— {isEs ? "Dra. Gabriella" : "Dra. Gabriella"}</footer>
            </blockquote>
