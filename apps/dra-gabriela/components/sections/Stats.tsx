          ...
          {display.map((it: any, i: number) => {
            ...
            return (
              <div key={i} className="text-center group">
                <div className="w-14 h-14 rounded-2xl bg-[var(--accent-soft)] flex items-center justify-center mx-auto mb-4 group-hover:bg-[var(--accent)] group-hover:scale-110 transition-all duration-300">
                  <Icon className="w-6 h-6 text-[var(--accent)] group-hover:text-white transition-colors duration-300" />
                </div>
                <div className="text-4xl md:text-5xl font-heading font-medium text-[var(--accent)] mb-1.5 leading-none" style={{ fontFamily: "var(--font-heading)" }}>
                  {it.value}
                </div>
                <div className="text-sm text-[var(--fg-muted)] leading-snug max-w-[180px] mx-auto">{it.label}</div>
                {it.note && (
                  <div className="text-xs text-[var(--fg-subtle)] mt-1.5 italic max-w-[180px] mx-auto">{it.note}</div>
                )}
              </div>
            )
          })}
