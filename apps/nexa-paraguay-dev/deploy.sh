#!/usr/bin/env bash
# ── Nexa Paraguay DEV — Swarm deploy (staging) ──
# Builds from apps/nexa-paraguay-dev/.next/standalone (already produced by
# `pnpm build` on the host) using Dockerfile.standalone, tags the image
# with the git short SHA + timestamp, and rolls the nexa-paraguay-dev_web
# service forward.
#
# Usage:
#   ./deploy.sh                    # build + rolling update
#   ./deploy.sh --no-build         # only re-tag + update
set -euo pipefail

APP_DIR="$(cd "$(dirname "$0")" && pwd)"
REPO_ROOT="$(cd "$APP_DIR/../.." && pwd)"

SERVICE="${SERVICE:-nexa-paraguay-dev_web}"
APP_NAME="nexa-paraguay-dev"
DO_BUILD=1
