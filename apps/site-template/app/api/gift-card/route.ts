...
import { businessData, siteConfig, PYG_USD_RATE, getSiteName } from "@/lib/config/config"
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [{
        price_data: {
          currency: "usd",
          product_data: {
            name: card_name || `Tarjeta de Regalo – ${getSiteName()}`,
            description: `Regalo de Gs. ${numAmount.toLocaleString("es-PY")}`,
          },
          unit_amount: Math.round(numAmount / PYG_USD_RATE * 100),
        },
        quantity: 1,
      }],
      success_url: `${process.env.NEXT_PUBLIC_BASE_URL || "https://tu-emprendimiento.com"}/es/tarjetas-de-regalo/comprar/success?session_id={CHECKOUT_SESSION_ID}&gift_card=true`,
      cancel_url: `${process.env.NEXT_PUBLIC_BASE_URL || "https://tu-emprendimiento.com"}/es/tarjetas-de-regalo?cancelled=1`,
      metadata: {
