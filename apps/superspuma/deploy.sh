#!/bin/bash
# superspuma deploy — Next.js standalone (uses local .next/standalone if fresh)
set -euo pipefail
cd "$(dirname "$0")"
[ -f .env ] && { set -a; source ./.env; set +a; }

DATE=$(date +%Y%m%d-%H%M)
TAG="superspuma:prod-${DATE}"
...
echo "--- deploy: superspuma_web (rolling update)"
docker service update --image "$TAG" superspuma_web
