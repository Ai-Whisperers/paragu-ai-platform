...
export function ShareButtons({ title, url, intro }: ShareButtonsProps) {
  ...
  return (
    <div className="border-t border-[var(--color-warm-deep)] pt-8 mt-12">
      {intro && <p className="text-sm text-text-light mb-4">{intro}</p>}
      <div className="flex flex-wrap items-center gap-2">
        <span className="text-xs uppercase tracking-wider text-text-muted font-medium mr-2">
          Compartir:
        </span>
        {links.map((l) => (
          <a
            ...
            className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-white text-xs font-medium ${l.color} transition-colors`}
          >
            {l.icon}
            {l.label}
          </a>
