      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center gap-1">
            {items.map(it => (
              <Link key={it.href} href={it.href} className="px-3 py-2 text-sm text-fg-muted hover:text-fg hover:bg-bg-elev rounded-md transition-colors">{it.label}</Link>
            ))}
          </div>
          <div className="flex items-center gap-2">
            ...
            <Link href={`${base}/contact`} className="hidden sm:inline-flex items-center gap-1 px-3 py-2 bg-accent text-white text-sm font-medium rounded-md hover:bg-accent/80 transition-colors">
              <MessageCircle className="w-4 h-4" /> Contact
            </Link>
            <button onClick={() => setOpen(!open)} className="md:hidden p-2 text-fg-muted hover:text-fg">
        ...
        {open && (
          <div className="md:hidden pb-4 space-y-1">
            {items.map(it => (
              <Link key={it.href} href={it.href} className="block px-3 py-2 text-sm text-fg-muted hover:text-fg hover:bg-bg-elev rounded-md">{it.label}</Link>
            ))}
          </div>
        )}
      </div>
