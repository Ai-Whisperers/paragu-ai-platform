#!/usr/bin/env bash
# ── Nexa Paraguay — Swarm deploy ──
# Builds from apps/nexa-paraguay/.next/standalone (already produced by
# `pnpm build` on the host) using Dockerfile.standalone, tags the image
# with the git short SHA + timestamp, and rolls the nexa-paraguay_web
...
#   SERVICE=nexa-preview_web ./deploy.sh   # update a different service
set -euo pipefail

APP_DIR="$(cd "$(dirname "$0")" && pwd)"
REPO_ROOT="$(cd "$APP_DIR/../.." && pwd)"

SERVICE="${SERVICE:-nexa-paraguay_web}"
APP_NAME="nexa-paraguay"
DO_BUILD=1
