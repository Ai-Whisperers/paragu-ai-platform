    ...
    <header style={{ background: '#1a1a2e', padding: '1rem 2rem', position: 'sticky', top: 0, zIndex: 50 }}>
      <div style={{ maxWidth: '80rem', margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          ...
          <span style={{ color: 'white', fontFamily: 'Georgia, serif', fontSize: '1.25rem', fontWeight: 700 }}>{nav?.businessName || 'Scott Tatuajes'}</span>
        </div>
          ...
          {items.slice(0, 8).map((img: any, i: number) => (
            <div key={i} style={{ aspectRatio: '1', background: '#e5e7eb', borderRadius: '0.75rem', overflow: 'hidden', position: 'relative' }}>
              <img src={img.url || img.src || `https://picsum.photos/400/400?random=${i}`}
                alt={img.caption || img.alt || 'Trabajo de tatuajes'}
...
function CTASection({ content }: SectionProps) {
  ...
  const msg = encodeURIComponent(contact.cta?.text || contact.whatsappMessage || 'Hola! Quiero reservar un turno en Scott Tatuajes');
  return (
    ...
    <footer style={{ background: '#1a1a2e', color: 'rgba(255,255,255,0.7)', padding: '3rem 2rem' }}>
      <div style={{ maxWidth: '80rem', margin: '0 auto' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '3rem', marginBottom: '3rem' }}>
          <div>
            <h3 style={{ color: 'white', fontSize: '1.25rem', marginBottom: '1rem', fontFamily: 'Georgia, serif' }}>{site.name || 'Scott Tatuajes'}</h3>
        ...
        <div style={{ borderTop: '1px solid rgba(255,255,255,0.1)', paddingTop: '2rem', textAlign: 'center' }}>
          <p style={{ fontSize: '0.875rem' }}>
            © {new Date().getFullYear()} {site.name || 'Scott Tatuajes'} · {site.city || 'San Lorenzo'}, {site.country || 'Paraguay'}<br />
