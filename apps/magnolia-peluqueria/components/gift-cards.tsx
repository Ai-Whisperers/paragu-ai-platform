  ...
  async function handleStripe() {
    ...
    try {
      const res = await fetch("/api/gift-card", {
        ...
        body: JSON.stringify({ amount: card.amountGs, card_name: card.name }),
      })
      ...
      if (data.waFallback) {
        ...
        window.open(waLink(`Hola! Quiero comprar una Tarjeta de Regalo: ${card.name} - ${card.price}`), "_blank")
      }
    } catch {
      window.open(waLink(`Hola! Quiero comprar una Tarjeta de Regalo: ${card.name}`), "_blank")
    } finally {
      setLoading(false)
    }
  }
    ...
    <ScrollReveal delay={index * 80} direction="down">
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-all overflow-hidden group flex flex-col">
        ...
        <div className="p-6 flex flex-col flex-1">
          ...
          <h3 className="font-heading text-lg font-bold text-primary text-center mb-1">{card.name}</h3>
          <p className="text-sm text-foreground-light text-center mb-1 flex-1">{card.desc}</p>
