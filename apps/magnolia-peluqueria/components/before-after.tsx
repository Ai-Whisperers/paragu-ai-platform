    ...
    <ScrollReveal delay={index * 150} direction="up">
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        >
          ...
          <img src={item.after} alt={`Despues: ${item.label}`} className="absolute inset-0 w-full h-full object-cover" />
          ...
          <div
            ...
            style={{ width: `${slider}%` }}
          >
            <img src={item.before} alt={`Antes: ${item.label}`} className="absolute inset-0 w-full h-full object-cover" style={{ width: `${100 / (slider / 100)}%` }} />
          </div>
        ...
        <div className="p-5 text-center">
          <h3 className="font-heading text-lg font-bold text-primary mb-1">{item.label}</h3>
