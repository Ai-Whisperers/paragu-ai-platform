...
import { Tag, Clock, ArrowRight, Gift, Sparkles, Users, ExternalLink } from "lucide-react";
...
function DynamicPromotionCard({ promo, index }: { promo: Promotion; index: number }) {
  const isExpiresSoon = promo.expires_at && new Date(promo.expires_at) <= new Date(Date.now() + 14 * 86400000);
