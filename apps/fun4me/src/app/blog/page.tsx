  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-10">
      ...
      <div className="flex gap-2 flex-wrap justify-center mb-10">
        <Link href="/blog" className="px-4 py-2 rounded-lg bg-primary text-primary-foreground text-sm font-medium">
          Todas
        </Link>
        {b.categories.map((cat: string) => {
          const slug = cat.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "")
          return (
          <Link key={cat} href={`/blog/categoria/${slug}`}
        ...
        {(b.posts as BlogPost[]).map((post) => (
          <Link key={post.slug} href={`/blog/${post.slug}`} className="group">
            <article className="rounded-xl border border-border bg-card overflow-hidden transition-all group-hover:-translate-y-1 group-hover:shadow-md">
              <div className="h-48 bg-gradient-to-br from-surface-light to-surface flex items-center justify-center overflow-hidden">
                {post.image ? (
              ...
              <div className="p-5">
                ...
                <h3 className="font-semibold text-base mb-2 group-hover:text-primary transition-colors line-clamp-2">{post.title}</h3>
